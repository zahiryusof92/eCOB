        ...
        <div class="single-page-block-inner effect-3d-element">
            ...
            <div class="single-page-block-form">
                ...
                <form>
                    ...
                    <div class="form-group">
                        <input type="password" class="form-control" placeholder="Kata Laluan *" id="password">
                        <div id="password_error" style="display:none;"></div>
                    </div>
                    <div class="form-group">
                        <input type="password" class="form-control" placeholder="Pengesahan Kata Laluan *" id="retype_password">
                    ...
                    <a href="{{URL::action('UserController@login')}}">Log Masuk</a>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    function register() {
        ...
        if (username.trim() === "") {
            $("#username_error").html('<span style="color:red;font-style:italic;font-size:13px;">Sila masukkan Username</span>');
        ...
        if (retype_password.trim() === "") {
            $("#retype_password_error").html('<span style="color:red;font-style:italic;font-size:13px;">Sila masukkan Pengesahan Kata Laluan</span>');
            $("#retype_password_error").css("display", "block");
            error = 1;
        }
        if (password.trim() !== retype_password.trim()) {
            $("#retype_password_error").html('<span style="color:red;font-style:italic;font-size:13px;">Pengesahan Kata Laluan salah</span>');
            $("#retype_password_error").css("display", "block");
            error = 1;
        }
        if (name.trim() === "") {
        ...
        if (phone_no.trim() === "" || isNaN(phone_no)) {
            $("#phone_no_error").html('<span style="color:red;font-style:italic;font-size:13px;">Sila masukkan No. Telefon</span>');
        ...
        if (error === 0) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.trim() === "true") {
                        bootbox.alert("<span style='color:green;'>Pendaftaran berjaya!</span>", function () {
                            window.location = '{{URL::action("UserController@login") }}';
                        });
                    } else if (data.trim() === "username_in_use") {
                        $("#username_in_use").html('<span style="color:red;font-style:italic;font-size:13px;">Username ini telah wujud</span>');
